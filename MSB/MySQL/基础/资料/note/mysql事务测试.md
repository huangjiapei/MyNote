...
5、测试2：当使用read committed的时候，就不会出现脏读的情况了，当时会出现不可重复读的问题（同一个事务中，多次查询结果不一致）
...
6、测试3：当使用repeatable read的时候(保证同一个事务读到的数据是一致的)，就不会出现不可重复读的问题，但是会出现幻读（插入数据产生幻读，两个事务都插入数据，以为数据不存在，貌似可以插入数据，实际在insert的时候，如果有重复主键会报错）的问题
